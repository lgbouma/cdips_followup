...
CALCULATE:
    ...
    remove_cosmic_rays: remove cosmics from a 1d extracted spectrum
...
def remove_cosmic_rays(
    ...
    """Identify and remove cosmic rays from a 1D spectrum via interpolation.

    This function uses a median filter to detect sharp cosmic ray spikes in the
    spectrum and replaces these points with interpolated values from the
    unaffected regions.

    Generated by ChatGPT o3-mini-high on March 17 2025.

    Args:
        wavelength (np.ndarray): Array of wavelength values.
        flux (np.ndarray): Array of flux values.
        sigma_threshold (float): Threshold in sigma for cosmic ray detection.
            Defaults to 5.0.
        window_size (int): Size of the median filter window. Defaults to 5.

    Returns:
        np.ndarray: Flux array with cosmic ray spikes replaced by interpolated
        values.
    """
    median_flux = median_filter(flux, size=window_size)
    diff = flux - median_flux
    std = np.std(diff)
    cosmic_rays = np.abs(diff) > (sigma_threshold * std)
    corrected_flux = np.copy(flux)
    corrected_flux[cosmic_rays] = np.interp(
        wavelength[cosmic_rays],
        wavelength[~cosmic_rays],
        flux[~cosmic_rays]
    )
