...
Each row in this CSV file is a planet candidate entry. This database is used to track
what needs observing, what has been observed, and whether more observations are
needed.

Use cases:

* Add candidates given source_id or ticid.
    * e.g., someone emails with information --> create new entry
    * include further information from ExoFOP-TESS uploads

* Update columns given new information
    * Update current_priority
    * Update pending_spectroscopic_observations
    * Update pending_photometry_observations
    * Update comment

* Export for google spreadsheet viewing.

* View spreadsheet to select viable candidates. Viewing can be done in
...
def insert_candidate(
    ...
    iscdipstarget = True if isinstance(cdips_r, pd.DataFrame) else False
    ...
    #
    # Get the fit information as uploaded to ExoFOP-TESS
    #
    ...
    new_row = pd.DataFrame({
        ...
        'comment': comment,
        'rp': plproperties_r['Planet Radius (R_Earth)'],
        'period': plproperties_r['Period (days)'],
        'depth': plproperties_r['Depth (mmag)'],
        'gaia_ra': cdips_r.ra,
...
def save_candidates_csv_file(cand_df):
    ...
    if not os.path.exists(CAND_DAY_PATH):
        out_df.to_csv(
            CAND_DAY_PATH, index=False, float_format='%.2f'
        )
        print('{}: backed up to {}'.
              format(datetime.utcnow(), CAND_DAY_PATH))

    out_df.to_csv(
        CAND_PATH, index=False, float_format='%.2f'
    )
